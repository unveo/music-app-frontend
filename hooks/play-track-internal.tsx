...
async function addToHistory(trackId: number) {
	await listeningHistoryService.add(trackId);
}

function prefetchNextTrackAudio() {
	const nextId = useQueueStore.getState().next[0];

	if (!nextId) {
		return;
	}

	trackService.getOneById(nextId).then(({ data }) => {
		const link = document.createElement('link');
		link.rel = 'prefetch';
		link.href = `${AUDIO_URL}/${data.audio}${AUDIO_ENDING}`;
		document.head.appendChild(link);
	});
}

export function usePlayTrackInternal() {
	const {
		isPlaying,
		audio,
		audioReady,
